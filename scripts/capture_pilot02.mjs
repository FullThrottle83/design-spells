...
// node scripts/capture_pilot02.mjs [before|after]
...
const variants=phase==='before'?[['no-preference','light']]:[['no-preference','light'],['reduce','light'],['no-preference','dark'],['reduce','dark']];
for(const width of [1440,390]) for(const [motion,scheme] of variants) for(const id of [111,93,134,30,55,69]){
 ...
 await page.evaluate(({id,phase})=>{
  ...
  if(phase==='after'&&target)scrollTo(0,target.getBoundingClientRect().top+scrollY-(id===111?innerHeight*.45:60));else scrollTo(0,document.body.scrollHeight/2);
 },{id,phase});
