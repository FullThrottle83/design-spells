...
Build, data and Chromium browser layers run by `npm test`. Firefox/WebKit have their own CI job.
...
The original browser layer includes two spec files: `previews.spec.mjs` (the preview sandbox)
...
## Native behavior, independent of the preview geometry checks

`native-behavior.spec.mjs` exercises named interactions in Chromium, Firefox and WebKit with client JavaScript disabled: category filtering (#120), exclusive native disclosures (#132), and form-validity-driven styling (#128). These tests check the actual state change, not just CSS parsing or nonzero box size. They do not imply full support across all 150 spells or WCAG conformance.

`public/classic/index.html` is a reproducible large build artifact, generated by `npm run build` and deliberately not tracked. The CI and deploy jobs both build it; `/classic/` stays available online.
